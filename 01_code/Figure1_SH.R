...
######------ Read in Allelic Ratios Major Cardiac Lineages (MCL) ------###### 
allelic_ratios_MCL <- read_excel(paste0(input,"SupplementaryTable_1.xlsx"), sheet = "d AllelicRatio_MCL")

# set and apply min_total_reads cutoff
min_MCL=10
allelic_ratios_MCL <- allelic_ratios_MCL %>% dplyr::filter(min_total_reads >= min_MCL)

# extract median data
allelic_ratios_MCL$sample <- gsub("_1","",allelic_ratios_MCL$sample)
allelic_ratios_MCL$sample <- gsub("_2","",allelic_ratios_MCL$sample)
allelic_ratios_MCL$sample <- gsub("_3","",allelic_ratios_MCL$sample)
allelic_ratios_MCL <- allelic_ratios_MCL %>% dplyr::select(-"allelic_ratio", -"total_reads")
allelic_ratios_MCL <- distinct(allelic_ratios_MCL)

######------ Read in TMPs  ------###### 
TPM_MCL <- read_excel(paste0(input,"SupplementaryTable_1.xlsx"), sheet = "e TPM_MCL")

# extract median data
TPM_MCL$sample <- gsub("_1","",TPM_MCL$sample)
TPM_MCL$sample <- gsub("_2","",TPM_MCL$sample)
TPM_MCL$sample <- gsub("_3","",TPM_MCL$sample)
TPM_MCL <- TPM_MCL %>% dplyr::select(-"TPM")
TPM_MCL <- distinct(TPM_MCL)
#-------------------------------------------------------------------------------------------------------------------------------------------------------------#


######---- Figure 1j: MCL Escape heatmap----###### 

# determine MCL male escape genes
escapers_XY <- allelic_ratios_MCL %>%dplyr::filter(chr == "chrX" & sex == "male" & median_allelic_ratio<=0.90)
drop_MCL <- c(unique(escapers_XY$name), "Xist", "Tsix")
  
escapees_He <- allelic_ratios %>% dplyr::filter(organ == "He" & sex == "female" & chr == "chrX" & ! name %in% drop & median_allelic_ratio<=0.90)
escapees_MCL <- allelic_ratios_MCL %>% dplyr::filter(sex == "female" & chr == "chrX" & ! name %in% drop_MCL & median_allelic_ratio<=0.90)

# Extract escape genes to plot in heatmap
escapees_He <- unique(escapees_He$name)
escapees_MCL <- unique(escapees_MCL$name)
Escapees <- unique(c(escapees_He, escapees_MCL))
...
# MCL
Heatmap_MCL <- allelic_ratios_MCL %>% dplyr::filter(sex == "female" & name %in% Escapees)
Heatmap_MCL <- Heatmap_MCL %>% dplyr::select(name, median_allelic_ratio, celltype)
colnames(Heatmap_MCL)[colnames(Heatmap_MCL) == "celltype"] <- "organ"
  
# combine whole heart and MCL
esc_Heatmap <- rbind(Heatmap_He, Heatmap_MCL)
...
######---- Figure 1k: Karyoplot for all escape genes across MCL ----###### 
Karyoplot_MCL <- allelic_ratios_MCL %>% dplyr::filter(sex == "female" & chr == "chrX" & ! name %in% drop_MCL & median_allelic_ratio<=0.90)
Karyoplot_He <- allelic_ratios %>% dplyr::filter(organ == "He" & sex == "female" & chr == "chrX" & ! name %in% drop & median_allelic_ratio<=0.90)

# get coordinates for all escapees
label_MCL <- Karyoplot_MCL %>%  dplyr::select("chr", "start", "end", "name")
label_He <- Karyoplot_He %>% dplyr::select("chr", "start", "end", "name")
label <- rbind(label_MCL, label_He)
label <- distinct(label)
label$chr <- gsub("chr","",label$chr)
genes_coord_all <- regioneR::toGRanges(as.data.frame(label)) 
seqlevelsStyle(genes_coord_all) <- "UCSC"
...
# separate celltypes
Karyoplot_CM <- Karyoplot_MCL %>%  dplyr::filter(celltype == "CM")
Karyoplot_FB <- Karyoplot_MCL %>%  dplyr::filter(celltype == "FB")
Karyoplot_EC <- Karyoplot_MCL %>%  dplyr::filter(celltype == "EC")
Karyoplot_MP <- Karyoplot_MCL %>%  dplyr::filter(celltype == "MP")
...
######---- Figure 1l: Venn diagram for whole heart and MCL escape genes ----###### 

#create euler
Venn_He_MCL <- list(escapees_He = escapees_He, escapees_MCL = escapees_MCL)

#plot
Figure_1l <- plot(euler(Venn_He_MCL), shape = "circle", quantities = TRUE, input = "disjoint", fills = "transparent", main = "Figure 1l")


######---- Figure 1m: Distance plot for MCL and whole He escapees ----###### 
Distance_MCL <- allelic_ratios_MCL %>% dplyr::filter(sex == "female" & chr == "chrX" & ! name %in% drop_MCL & median_allelic_ratio<=0.90)
...
Distance_MCL <- Distance_MCL %>%  dplyr::select(name, start, end)
Distance_MCL <- distinct(Distance_MCL)
Distance_MCL <- Distance_MCL %>%dplyr:: filter(! name %in% Distance_He$name)


# Function to calculate shortest distance
calculate_shortest_distance <- function(mcl_row, Distance_He) {
  distances_start <- abs(mcl_row$start - Distance_He$start)
  distances_end <- abs(mcl_row$end - Distance_He$end)
  distances_3 <- abs(mcl_row$start - Distance_He$end)
  distances_4 <- abs(mcl_row$end - Distance_He$start)
  min_distance <- min(distances_start, distances_end, distances_3, distances_4)
  return(min_distance)
}

# Create a new column to store the shortest distance in Distance_MCL
Distance_MCL$distance <- NA

# LOOP to calculate the shortest distance
for (i in 1:nrow(Distance_MCL)) {
  #loop through rows of MCL-specific escapees
  mcl_row <- Distance_MCL[i, ]
  ...
  shortest_distances <- sapply(1:nrow(Distance_He), function(j) {
    calculate_shortest_distance(mcl_row, Distance_He[j, ])
  })
  #define which values of the 12 rows is the smallest and assign to new column
  shortest_distance <- min(shortest_distances)
  Distance_MCL$distance[i] <- shortest_distance
}

# filter for distance <= 2500000
Distance_MCL <- Distance_MCL %>% dplyr::arrange(Distance_MCL$distance) %>% dplyr::filter(distance <= 2500000) #8/9 are <= 2500000: use later for pie plot
# manually add direction and closest whole heart escape gene
Distance_MCL$position <- c("positive", "negative", "negative", "positive", "positive", "negative", "negative", "positive")
Distance_MCL$He_escapee <- c("Utp14a", "Kdm5c", "Pbdc1","Ddx3x", "Ddx3x", "Ddx3x", "Ddx3x", "4930578C19Rik" )


Distance_MCL <- Distance_MCL %>%
...
Distance_MCL$scatter <- rownames(Distance_MCL)
Distance_MCL$plot_distance <- as.numeric(Distance_MCL$plot_distance)
Distance_MCL$scatter <- as.numeric(Distance_MCL$scatter)
Distance_MCL$label <- paste0(Distance_MCL$He_escapee,"_",Distance_MCL$name,"_",Distance_MCL$plot_distance)

# plot
Figure_1m_scatter <- ggplot(Distance_MCL, aes(x= plot_distance, y= scatter)) +
  geom_point(position = "identity") +
  scale_x_continuous(limits= c(-2500000,2500000))+
  scale_y_continuous(limits= c(0,9))+
  labs(title = "Figure 1m", x = "Distance between MCL-specific and Heart escapees [Mb]", y = "") +
